...
{
public:
  CPlexServer(const std::string& uuid, const std::string& name, bool owned, bool synced = false)
    : m_owned(owned), m_home(false), m_synced(synced), m_uuid(uuid), m_name(name), m_lastRefreshed(0) {}
  ...
  bool CollectDataFromRoot(const std::string xmlData);
  ...
  std::string GetServerClass() const { return m_serverClass; }
  ...
  bool Equals(const CPlexServerPtr& otherServer) { return m_uuid == otherServer->m_uuid; }
  ...
  CURL BuildURL(const std::string& path, const std::string& options="") const;
  CURL BuildPlexURL(const std::string& path) const;
  ...
  void SetServerClass(const std::string& classStr) { m_serverClass = classStr; }
